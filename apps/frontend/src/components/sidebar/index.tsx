...
export function Sidebar({ closeSidebar, isCollapsed = false, toggleCollapse }: SidebarProps) {
  ...
  return (
    <aside className={`h-full flex-shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white flex flex-col p-4 transition-all duration-300 ${isCollapsed ? 'w-24' : 'w-64'}`}>
      <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} mb-10 overflow-hidden`}>
        <button 
          onClick={toggleCollapse} 
          className="text-2xl font-bold text-teal-600 dark:text-teal-400 focus:outline-none"
